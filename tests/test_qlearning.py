...
def q_table_learned(env: GridWorld) -> np.ndarray:
    """Provides a Q-table learned with a sufficient number of episodes
    for tests to pass."""
...
def test_greedy_policy_reaches_goal(q_table_learned: np.ndarray, env: GridWorld):
    ...
    for _ in range(num_trials):
        ...
        while (
            ...
            # print(
            #     f"  Step {steps_this_trial + 1}: State {current_state_tuple} "
            #     f"(ID: {state_id}), Action: {action}"
            # )
            ...
            if done and current_state_tuple == env.goal:
                ...
                # Reached terminal state but not THE goal
                # (should not happen with this env)
                # print(
                #     f"  Terminal state {current_state_tuple} reached but not goal "
                #     f"after {steps_this_trial} steps."
                # )
                break
    ...
    assert successful_reaches >= min_successful_trials, (
        f"Greedy policy only reached goal in {successful_reaches}/{num_trials} "
        f"trials. Expected at least {min_successful_trials}."
    )
    ...
    if successful_reaches > 0:
        ...
        assert average_steps <= max_steps_to_goal, (
            f"Greedy policy reached goal in {average_steps:.2f} steps on average, "
            f"expected <= {max_steps_to_goal}."
        )
    # If successful_reaches is 0 (and less than min_successful_trials),
    # the first assert would have failed.
