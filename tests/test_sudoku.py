...
from sudoku.csp import all_units, peers_map, ac3, Cell
...
def test_ac3_shrinks_domains():
    ...
    domains = {(r, c): ({EASY_PUZZLE.grid[r][c]} if EASY_PUZZLE.grid[r][c] is not None else set(range(1, 7)))
              for r in range(EASY_PUZZLE.size) for c in range(EASY_PUZZLE.size)}
